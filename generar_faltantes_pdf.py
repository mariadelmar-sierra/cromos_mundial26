...
def crear_pdf_faltantes(df, titulo="Lista de cromos faltantes"):
    """
    Crea un PDF en memoria con los cromos que faltan.
    ...
    faltantes = df[df["lo_tengo"] == False].copy()
    ...
    story.append(
        Paragraph(
            ...
            f"{total_faltantes} cromo(s) faltante(s) en total",
            subtitle_style,
        )
    )
    ...
    if faltantes.empty:
        ...
        story.append(Paragraph("No hay cromos faltantes actualmente.", styles["Normal"]))
        doc.build(story)
        buffer.seek(0)
        return buffer.getvalue()

    # Orden de selecciones exactamente como aparece en la app/dataset.
    selecciones_ordenadas = list(dict.fromkeys(faltantes["seleccion"].dropna().tolist()))

    for seleccion in selecciones_ordenadas:
        bloque = faltantes[faltantes["seleccion"] == seleccion].copy()
        bloque = bloque.sort_values("orden_original")

        total_sel = len(bloque)

        story.append(
            Paragraph(
                f"{seleccion} - {total_sel} faltante(s)",
                section_style,
            )
        )
...
def guardar_pdf_faltantes_desde_csv(
    csv_path="album_guardado.csv",
    output_path="faltantes_album.pdf",
):
    """
    Uso como script independiente:
        python generar_faltantes_pdf.py

    Lee album_guardado.csv y crea faltantes_album.pdf.
    """
    df = pd.read_csv(csv_path)
    pdf_bytes = crear_pdf_faltantes(df)
...
if __name__ == "__main__":
    ruta = guardar_pdf_faltantes_desde_csv()
